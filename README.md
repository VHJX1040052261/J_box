...
- **`off_topic` 的判据完全跟着 `scope` 的措辞走，而且这是稳定偏差、不是抖动。** 同一段明明在范围内的请求（"改掉引用了已删除模块的 import，再跑类型检查"），各跑 5 次：scope 写成抽象目标"收敛仓库并推送"时得 **0.71 / 0.74 / 0.72 / 0.75 / 0.73**（跨度 0.04），scope 列举出具体动作时得 **0.06 五次不变**。也就是说这一路是在拿 `scope` 自己的词表做匹配，不是真的语义包含；**k 次采样取中位数救不了它**，只能把 `scope` 写成可枚举的动作清单。这条已经用特征测试钉在 `test.mjs` 里（断言的是"已知会错"，模型哪天改了会暴露出来）。
